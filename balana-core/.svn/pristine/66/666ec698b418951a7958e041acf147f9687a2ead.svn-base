...
public class AnyOfSelection {
    ...
    /**
     * The AllOf elements contained in the AnyOf element.
     */
...
//            return new MatchResult(MatchResult.MATCH);
	...
	/**
	 * @param otherAnyOfSelection the AnyOf element to compare
	 * @return the intersection between two AnyOf elements
	 */
	public Set<TargetMatch> intersection(AnyOfSelection otherAnyOfSelection) {
		Set<TargetMatch> result = new HashSet<TargetMatch>();
		for(int i = 0; i < allOfSelections.size(); i++) {
			for(int j = 0; j < otherAnyOfSelection.getAllOfSelections().size(); j++) {
				result.addAll(allOfSelections.get(i).intersection(
						otherAnyOfSelection.getAllOfSelections().get(j)));
				//System.out.println(allOfSelections.get(i) + " - " + otherAnyOfSelection.getAllOfSelections().get(j));
			}
		}
		return result;
	}
	...
	/**
	 * Checks whether the intersection between this AnyOfSelection and
	 * the specified one is empty.
	 * @param otherAnyOfSelection the AnyOf element to compare
	 * @return <code>true</code> if the intersection is an empty set
	 */
	public boolean isIntersectionEmpty(AnyOfSelection otherAnyOfSelection) {
		return intersection(otherAnyOfSelection).size() == 0;
	}
	
	/**
	 * @param other the <code>AnyOfSelection</code> element to merge
	 * @return a new <code>AnyOfSelection</code> which properly combines all the
	 * <code>AllOfSelection</code> elements of this object with the ones of the
	 * specified <code>AnyOfSelection</code>.
	 */
	public AnyOfSelection merge(AnyOfSelection other) {
		List<AllOfSelection> allOfs = new ArrayList<AllOfSelection>();
		for(int i = 0; i < allOfSelections.size(); i++)
			for(int j = 0; j < other.getAllOfSelections().size(); j++)
				allOfs.add(allOfSelections.get(i).merge(other.getAllOfSelections().get(j)));
		return new AnyOfSelection(allOfs);
	}
}
