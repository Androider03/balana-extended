...
public class ConflictAnalyzerGUI extends JFrame {
	...
	private ArrayList<File> policiesFilesList1;
	private ArrayList<File> policiesFilesList2;
	...
	public ConflictAnalyzerGUI(String title, int width, int height) throws HeadlessException {
		...
		conflicts = null;
		policiesFilesList1 = new ArrayList<File>();
		policiesFilesList2 = new ArrayList<File>();
		...
		/*TableColumn myTableColumn6 = conflictsTable.getColumnModel().getColumn(6);
		TableColumn myTableColumn7 = conflictsTable.getColumnModel().getColumn(7);
		conflictsTable.getColumnModel().removeColumn(myTableColumn6);
		conflictsTable.getColumnModel().removeColumn(myTableColumn7);*/
	...
	private class RunListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for(int i = 0; i < conflicts.size(); i++) {	
				...
				policiesFilesList1.add(firstRule.getPolicy().getPolicyFile());
				policiesFilesList2.add(secondRule.getPolicy().getPolicyFile());
			...
			conflictsTable.addMouseListener(new TableMouseListener());
	...
	private void drawPoliciesStatisticsChart() {
        ...
        ArrayList<PoliciesWithSameNumberOfConflicts> groupsOfPolicies = 
        		conflictsCollection.computeOrderedGroupsOfPoliciesWithSameNumberOfConflicts(ConflictsCollection.DESCENDING_ORDER);
        ...
        if(tabs.getTabCount() > 2)
        	tabs.remove(2);
        tabs.addTab("Policies grouped by conflicts", policiesGraphicsPanel);
	}
	...
	private void drawRulesStatisticsChart() {
        ...
        ArrayList<RulesWithSameNumberOfConflicts> groupsOfRules = 
        		conflictsCollection.computeOrderedGroupsOfRulesWithSameNumberOfConflicts(ConflictsCollection.DESCENDING_ORDER);
        ...
        if(tabs.getTabCount() > 3)
        	tabs.remove(2);
        tabs.addTab("Rules grouped by conflicts", rulesGraphicsPanel);
	}
	...
	private class TableMouseListener extends MouseAdapter {
		...
		public void mouseReleased(MouseEvent e) {
			...
			if(0 <= col && col <= 3) {
				if(col == 0 || col == 1)
					policyFile = policiesFilesList1.get(row);
				else policyFile = policiesFilesList2.get(row);
				open(policyFile);
			}
		...
		/**
		 * Opens a file using default program.
